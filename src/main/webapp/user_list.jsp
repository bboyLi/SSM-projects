								...
								<table id="sample-table-1" class="table table-striped table-bordered table-hover dataTable">
									...
									<tbody>
										...
										<tr>
											...
											<td><font name="menuName">${item.username}</font></td>
											<c:if test="${item.role eq 1}">
												<td>系统管理员</td>
											</c:if>
											<c:if test="${item.role eq 2}">
												<td>服务员</td>
											</c:if>
											<c:if test="${item.role eq 3}">
												<td>移动用户</td>
											</c:if>
											<c:if test="${item.role eq 4}">
												<td>普通用户</td>
											</c:if>
											<td>
												<div class="visible-md visible-lg hidden-sm hidden-xs action-buttons">
													<a class="blue buy"  href="/user_delUser?id=${item.id}" data-rel="tooltip" data-original-title="删除" data-placement="top">
